...
test("المرحلة ٣: لكل حالة من حالات نَسَق عنصرها وتسميتها", () => {
  ...
  const labels = {
    ...
    // بلا «والتحسين»: العقد شكلٌ فقط (ملاحظة حارس نَسَق في m6؛ صُحّحت في ٢٦ إطارًا في Figma)
    result: "تم التنسيق بنجاح",
...
test("m6-16: واجهة نَسَق لا تَعِد بأكثر من الشكل — لا «تشكيل» ولا «تحسين»", () => {
  // عقد نَسَق شكلٌ وتنظيم وإخراج فقط (`nasaq/contracts.rs`). «تشكيل» تُفهم إضافةَ حركات، و«تحسين» تدخّلًا في
  // الأسلوب؛ وكلتاهما جاءتا من نصوص الملف فصُحّحتا فيه ثم هنا
  for (const [name, code] of [["index.html", html], ["nasaq.js", nasaq]]) {
    const ui = code.replace(/\/\/[^\n]*|\/\*[\s\S]*?\*\/|<!--[\s\S]*?-->/g, "");
    assert.ok(!/تشكيل|التحسين|تحسين النص/.test(ui), `صياغة تَعِد بما لا يضمنه العقد في ${name}`);
  }
});
...
test("المرحلة ٦: نصوص ورقة الترحيب من التصميم", () => {
  ...
  for (const text of [
    ...
    "أداة نَسَق",
    // «شكل» لا «تشكيل»: الثانية تُفهم إضافةَ حركات ولا يضمنها عقد نَسَق (ملاحظة حارسه في m6؛ صُحّحت في Figma 2008:16)
    "تُعنى بضبط شكل النص وإيقاعه وتجميل المظهر البصري للكلمات.",
    "أداة شَذْب",
