...
// مؤشر السطر الفارغ في حقول الكتابة العربية (rtl-caret.js): الحقل الرئيسي
// مشترك بين البرجين، فيُربط هنا مرة واحدة لا في أيٍّ منهما. تحسينٌ للرسم لا
// أكثر: تعذُّره يترك المؤشر الأصلي ولا يُسقط القشرة وما بعدها
try {
  for (const field of document.querySelectorAll('textarea[dir="rtl"]')) window.NasaqRtlCaret?.attach(field);
} catch (error) {
